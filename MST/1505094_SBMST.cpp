...
public:
	...
	void decreaseKey(int data, int key)
	{
		//Write your codes here.
		float temp;
		int d = map[data];
...
void Graph::setnVertices(int n)
{
	...
	parentNode = new int[nVertices+1];
...
/*intvoid*/int Graph::primsAlgorithm(int r){
...
int Graph::SBMST(){
    int s = primsAlgorithm(1);
    for(int i = 2; i <= nVertices;i++){
        edges[i].source = parent[i];
        edges[i].dest = i;
        edges[i].cost = weight[parent[i]][i];
    }
    int m = MAXREAL;
    for(int i = 2; i <= nVertices;i++){
        removeEdge(edges[i].source,edges[i].dest);
        if(isConnected() != true){
            addEdge(edges[i].source,edges[i].dest,edges[i].cost);
            continue;
        }
        int a = primsAlgorithm(1);
        if(m > a)
            m = a;
        addEdge(edges[i].source,edges[i].dest,edges[i].cost);
...
int main()
{
    int n,e,u,v,w;
    FILE *fp;
    fp = fopen("/Users/User/Desktop/1505094/input.txt","r");
    Graph g;
   ...
   /*for(int i = 0; i < e; i++){
        g.removeEdge(edges[i].source,edges[i].dest);
        if(!g.isConnected()){
            continue;
        }
        else{
